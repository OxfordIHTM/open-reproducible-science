...
# How about other IDEs for R?

* RStudio is not the only IDE that can be used with R: `Positron`, `vim`, `nvim`, `BBEdit` (for macOS only), `Sublime Text`, `VS Code`

* `Positron` (https://positron.posit.co/) - developed by the same company that created RStudio - is the most talked about within the R community recently and is being touted as an RStudio replacement by its developers.

* However, RStudio remains the go-to IDE of most IHTM instructors/teachers/supervisors.

???

Of the teachers and supervisors we know of in IHTM, most still use RStudio and we predict that this will still be the case for the next couple of years. This is the main reason why the module is still recommending and supporting the use of RStudio.
...
* GitHub has slowly gained notoriety and has drawn the ire of its users since it was acquired/bought by Microsoft in October 2018.

* Currently, the main issues that a number of stalwarts of the open source community have against GitHub are: 

     * with how it has allowed Microsoft (and subsequently OpenAI) to unethically mine the open source code available from GitHub without appropriate attribution and recognition of the open source licenses of the millions of open source code available from GitHub's repositories; and,
     * it's punitive actions against developers, most of whom have been members of the GitHub community since its early days, for their strong and public criticism of GitHub/Microsoft's aggressive peddling of **Co-pilot** and other AI services including **xAI**, Elon Musk's AI/LLM service.


???

A lot of early adopters and users of GitHub prior to the purchase by Microsoft felt that GitHub has copped out on its commitments to the open source community by going to bed with one of the most closed and proprietary software maker in the history of the industry and for its uncritical adoption of AI services that include xAI
